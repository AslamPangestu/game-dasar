using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class CreateItem : MonoBehaviour {

    public GameObject item;
    public int cost = 100;
    private Vector3 screenPoint;
    private Vector3 offset;
    private void OnMouseDown()//saat diklik
    {
        if (Data.coin > cost)//jika koin masih cukup untuk membuat item
        {
            Debug.Log("Create Item");
            GameObject _item = (GameObject)Instantiate(item, transform.position, Quaternion.identity);//membuat item
            foreach (Behaviour childCompnent in _item.GetComponentsInChildren<Behaviour>())
                childCompnent.enabled = false;//menonaktifkan behaviour 
            //mencatat posisi awal
            screenPoint = Camera.main.WorldToScreenPoint(gameObject.transform.position);
            offset = gameObject.transform.position - Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x,
            Input.mousePosition.y, screenPoint.z));
            //mengirim nilai posisi awal item dan mouse serta cost
            DragDrop dd = _item.GetComponent<DragDrop>();
            dd.enabled = true;
            dd.screenPoint = screenPoint;
            dd.offset = offset;
            dd.cost = cost;
        }
        else
        {
            Debug.Log("Koin Tidak cukup");
        }
    }
}
